...
- No Co-Authored-By trailers (e.g. Cursor); author commits as the user only
- After committing, run `git log -1 --format=full` to verify no Cursor trailer slipped in
- `.githooks/commit-msg` strips Cursor co-author lines; enable via `npm run setup` or `git config core.hooksPath .githooks`
